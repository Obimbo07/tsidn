...
function Navbar() {
  ...
  const links = [
    ...
    },
    {
      ...
      text: "TSDN Aims",
      url: "/tsdn-aims",
      dropdown: [{ id: 1, text: "What We Do", url: "/what-we-do" }],
    },
    {
      id: 3,
      text: "Themes",
            ...
            <li key={link.id} className="relative">
              <div className="flex items-center">
                ...
                {link.dropdown && (
                  ...
                  >
                    <FontAwesomeIcon
                      icon={openDropdown === link.id ? faAngleUp : faAngleDown}
              ...
              {link.dropdown && openDropdown === link.id && (
                <div
                  id={`dropdown-${link.id}`}
                  className="w-max md:absolute z-50 bg-white shadow-lg rounded mt-2"
                >
                  <ul>
                    {link.dropdown.map((item) => (
                      <li key={item.id} className="px-4 py-2 hover:bg-gray-200">
                        <Link href={`${link.url}${item.url}`}>{item.text}</Link>
                      </li>
                    ))}
                  </ul>
                </div>
